...
/**
 * 使用条件：</br>JAR包：jacob-1.18.jar[详见pom.xml]</br>软件：Office2010</br>DLL文件：jacob-1.18-x64.dll</br>JDK版本：1.7及以上
 * 
...
public class OfficeToPdf2 {
	...
	static Logger log = Logger.getLogger(OfficeToPdf2.class.getName());

	public static void main(String[] args) {
		System.out.println(toPdf("D:/EclipseWorkspace/AAA/p53_kjsch/Document/灵向/灵向科技智慧班牌方案（最新）.doc"));
	}
	...
	public static synchronized String toPdf(String inputFile) {
		if (UtilString.isBlank(inputFile)) {
			log.debug("转Pdf文件失败:文件路径为空");
			return null;
		}
		File file = new File(inputFile);
		if (!file.exists()) {
			log.debug("转Pdf文件失败:输入的路径“" + inputFile + "”不存在");
			return null;
		}
